  ...
  return (
    <div className={`flex gap-3 ${nested ? "ml-10 mt-3" : ""}`}>
      <img
        src={comment.avatar}
        alt={comment.name}
        className={`rounded-full object-cover flex-shrink-0 ${nested ? "w-7 h-7" : "w-9 h-9"}`}
      />
      ...
      <div className="w-full bg-[#f5f7fb] min-h-screen font-['Mada']">
        ...
        >
          ...
          <div className="absolute inset-0 flex items-center justify-center gap-20 select-none pointer-events-none overflow-hidden opacity-[0.04]">
            <span className="text-white font-black tracking-widest whitespace-nowrap text-[90px]">
              COLLABORATION
            </span>
            <span className="text-white font-black tracking-widest whitespace-nowrap text-[90px]">
              DEEP WORK
          ...
          <div className="absolute inset-0 flex items-center justify-center gap-3">
            {ARTICLE.categories.map((cat) => (
              <span
                key={cat}
                className="bg-white/15 backdrop-blur-sm text-white text-xs font-black tracking-widest px-5 py-2 rounded-full border border-white/25 uppercase"
              >
                {cat}
              </span>
            ))}
        ...
        <ContentWrapper>
          <div className="-mt-12 relative z-10 pb-16">
            ...
            <div className="px-10 pt-10 pb-6 border-b border-gray-50">
              <h1
                className="text-[26px] font-bold leading-snug mb-6"
                style={{
                  color: "#000000",
                  fontWeight: 500,
              ...
              <div className="flex items-center justify-between flex-wrap gap-4">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  {ARTICLE.tags.map((tag) => (
                    <span
                      key={tag}
                      className="text-xs font-semibold text-[#2563eb] bg-blue-50 px-3 py-1 rounded-full"
                    >
                      {tag}
                    </span>
                  ))}
            ...
            <div className="px-10 py-8 flex flex-col gap-6">
              ...
              {ARTICLE.paragraphs.map((p, i) => (
                <p key={i} className="text-[15px] text-gray-600 leading-8">
                  {p}
                </p>
              ))}
            </div>
