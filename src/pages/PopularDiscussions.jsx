...
function Toggle({ checked, onChange }) {
  return (
    ...
    >
      <span
        className={`absolute top-[3px] w-[18px] h-[18px] rounded-full bg-white shadow transition-all duration-200 ${checked ? "left-[23px]" : "left-[3px]"}`}
      />
    </button>
  );
}
...
export default function PopularDiscussions() {
  ...
  const updateFmt = () => {
    ...
    setFmt({
      bold: !!node.closest?.("b,strong"),
      italic: !!node.closest?.("i,em"),
    });
  };
  const toggleTag = (tag) =>
    setActiveTags((prev) =>
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      industry,
      images: [],
      groupId: "f30e50c1-1061-4ddf-bb2b-19ef98a8cd3e",
      isPublic,
      tags: activeTags.map((t) => t.replace("#", "")),
    };
      ...
      <div className="fixed inset-0 bg-black/20 flex items-center justify-center z-[1000] font-['Inter',sans-serif]">
        <div className="bg-white rounded-2xl shadow-2xl p-10 flex flex-col items-center gap-4">
          ...
          <p className="text-lg font-semibold text-gray-900">
            {isDiscussion ? "Discussion Created!" : "Post Published!"}
          </p>
    ...
    <div className="fixed inset-0 bg-black/20 flex items-center justify-center z-[1000] p-5 font-['Inter',sans-serif]">
      <div className="w-full max-w-[900px] bg-white rounded-2xl shadow-2xl p-7">
        ...
        <div className="flex items-center gap-3 mb-4">
          <button
            onClick={() => navigate(-1)}
            className="p-1 bg-transparent border-none cursor-pointer flex items-center justify-center hover:bg-gray-100 rounded-lg transition-colors"
          >
            <svg
              width="16"
              height="16"
              ...
              <div className="flex items-center justify-between gap-2">
                <div>
                  ...
                  <p className="text-[11px] text-gray-400 mt-0.5">
                    Visible to everyone on Gronet
                  </p>
                </div>
                <Toggle checked={isPublic} onChange={setIsPublic} />
              </div>
              <p className="text-[11px] font-semibold text-gray-400 tracking-wider uppercase mt-5 mb-2.5">
                Suggested Tags
              </p>
        ...
        <div className="flex items-center justify-between mt-6 pt-4 border-t border-slate-100">
          ...
          <button className="text-sm text-gray-900 bg-transparent border-none cursor-pointer hover:text-gray-600 transition-colors">
            Save as Draft
          </button>
          <button
